...
interface VendorBuyClicksResponse {
...
export class GA4Client {
  ...
  public async getUniqueUsers(
    ...
    const previousStartDate = includePreviousPeriod
      ? new Date(startDate.getTime() - periodLength)
      : null;
    const previousEndDate = includePreviousPeriod
    ...
    const totalUniqueUsers = parseInt(
    ...
    const previousTotalUniqueUsers = parseInt(
      previousTotalResponse.rows?.[0]?.metricValues?.[0]?.value || '0',
      10
    );

    // Calculate percentage change
    const percentageChange =
      previousTotalUniqueUsers > 0
        ? ((totalUniqueUsers - previousTotalUniqueUsers) /
            previousTotalUniqueUsers) *
          100
        : 0;
  ...
  public async getUsersByDevice(
    startDate: Date,
    endDate: Date = new Date()
  ): Promise<UsersByDeviceData> {
    try {
      const [response] = await this.client.runReport({
        property: `properties/${GA4_PROPERTY_ID}`,
        dateRanges: [
  ...
  public async getPopularBuyClicks(numberOfDays: number = 30) {
    const endDate = new Date();
    const startDate = subDays(endDate, numberOfDays);
  ...
  public async getSearchQueries(
    ...
    const percentageChange =
      previousTotalSearches > 0
        ? ((totalSearches - previousTotalSearches) / previousTotalSearches) *
          100
        : 0;
  ...
  public async getSearchQueriesWithParams(
    startDate: Date,
    endDate: Date = new Date(),
    includePreviousPeriod: boolean = false
  ) {
  ...
  public async getBuyClicksWithParams(
    startDate: Date,
    endDate: Date = new Date(),
    includePreviousPeriod: boolean = false
  ) {
    ...
    const averageDailyBuyClicks =
      Math.round((totalBuyClicks / numberOfDays) * 10) / 10;
    ...
    // Calculate percentage change
    const percentageChange =
      previousTotalBuyClicks > 0
        ? ((totalBuyClicks - previousTotalBuyClicks) / previousTotalBuyClicks) *
          100
        : 0;
  ...
  public async getVendorBuyClicks(
    startDate: Date,
    endDate: Date,
    limit: number = 5
  ): Promise<VendorBuyClicksResponse> {
    const [response] = await this.client.runReport({
      property: `properties/${GA4_PROPERTY_ID}`,
      dateRanges: [
        {
    ...
    buyClickData.forEach(({ website, tcg, count }) => {
      ...
      if (tcg in groupedData[website]) {
        (groupedData[website][
          tcg as keyof (typeof groupedData)[string]
        ] as number) += count;
      }
  ...
  public async getEngagementTime(
    startDate: Date,
    endDate: Date = new Date(),
    includePreviousPeriod: boolean = false
  ) {
    ...
    const averageEngagementTime = parseFloat(
    ...
    const previousAverageEngagementTime = parseFloat(
      previousResponse.rows?.[0]?.metricValues?.[0]?.value || '0'
    );

    // Calculate percentage change
    const percentageChange =
      previousAverageEngagementTime > 0
        ? ((averageEngagementTime - previousAverageEngagementTime) /
            previousAverageEngagementTime) *
          100
        : 0;
      ...
      data: [
        {
          ...
          percentage:
            total > 0
              ? Math.round(((data?.newUsers || 0) / total) * 1000) / 10
              : 0
        },
        {
          type: 'Returning',
          ...
          percentage:
            total > 0
              ? Math.round(((data?.returningUsers || 0) / total) * 1000) / 10
              : 0
        }
      ],
      total
  ...
  public async getUserRetention() {
    ...
    const retentionData = Object.entries(cohortData).map(
      ([cohortId, { data }]) => {
        ...
        const monthlyRetention = Object.entries(data).map(([month, users]) => ({
          ...
          users,
          percentage:
            initialUsers > 0 ? Math.round((users / initialUsers) * 100) : 0
        }));
  ...
  public async getPopularClickedCards(
    startDate: Date,
    endDate: Date = new Date()
  ) {
    try {
  ...
  public async getPopularClickedSets(
    startDate: Date,
    endDate: Date = new Date()
  ) {
    try {
