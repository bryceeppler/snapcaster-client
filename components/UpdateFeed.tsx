    ...
    <section className="w-full py-6 md:py-12 flex justify-center">
      <div className="container grid md:px-6 items-start gap-6">
        <div className="space-y-2">
          ...
          <p className="text-zinc-500 dark:text-zinc-400">Stay up to date with the latest changes.</p>
        </div>
      <div className="grid gap-4 md:gap-4 ">
          {props.updates.map((update, index) => (
            <div key={index} className="outline outline-1 outline-zinc-500 rounded-md p-4 md:p-8 grid gap-4 md:gap-2 text-left">
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                <h3 className="font-semibold overflow-hidden overflow-ellipsis tracking-tight">{update.title}</h3>
                  <p className="text-sm text-zinc-500 dark:text-zinc-400">{update.date}</p>
                </div>
