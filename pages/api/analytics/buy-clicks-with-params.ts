...
export default async function handler(
  ...
  try {
    console.log('Received request for buy-clicks-with-params:', req.query)
    ...
    console.log(`Fetching buy clicks with params from ${start.toISOString()} to ${end.toISOString()}, includePreviousPeriod: ${includePreviousPeriod}`)
    
    const result = await ga4Client.getBuyClicksWithParams(start, end, includePreviousPeriod)
    console.log('Successfully retrieved buy clicks with params')
    ...
    console.error('Error fetching buy clicks with parameters:', error)
